...
function loadStories(){
    ...
    SAMConnectAdmin_postToAEP({
		...
		if (typeof a == "object"){
			...
			jQuery('#samStoryContent').append('<div id="samAssetSelectBar"><div id="samAssetSelected"></div><div id="samAssetInsert"><button id="samClearSelect" type="button" class="button button-large" style="margin-right:15px;">Clear</button><button id="samInsertAsset" type="button" class="button button-primary button-large">Insert Into Post</button></div></div>');
			...
			jQuery.each(a,function(ind,ele){
				...
				storyHTML += '" id="story' + ele.id + '" data-storyid="' + ele.id + '">';
...
function bindAssetClicks(){
	...
	jQuery('.assetFull').click(function(){
		...
		if (jQuery(this).hasClass('selected')){
			...
			// Check if it's the last selected asset
			if (!jQuery('#samStoryAssetWrap .assetFull.selected').length) {
			...
			setTimeout(function(){
				...
				jQuery('#samAssetSelectBar #samInsertAsset').unbind('click').click(function(){
					...
					var assets = jQuery('#samStoryAssetWrap .assetFull.selected');
					if (assets.length) {
						...
						assets.each(function() {
							...
							var embedId = asset.attr('data-embedid');
							var socialType = asset.attr('data-socialtype');

							// Direct to the point!
							if (embedId && socialType){
								r += '[SAMASSET embedid="' + embedId + '" socialtype="' + socialType + '"]'
							}

						});

						if (r) {
							sendToEditorIncRender(r);
						}
